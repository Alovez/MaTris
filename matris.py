    ...
    def update(self, timepassed):
        ...
        for y in range(self.size['height']):
            for x in range(self.size['width']):
                ...
                block_location = Rect(x * self.blocksize, (y * self.blocksize - 2 * self.blocksize), self.blocksize,
                                      self.blocksize)
            ...
            for y in range(posY, posY + len(shape)):
                if (copy.get((y, x), False) is False and shape[y - posY][x - posX]  # shape is outside the matrix
                    ...
                    if allow_failure:
                        ...
                        raise BrokenMatrixException(
                            "Tried to blend a broken matrix. This should mean game over, if you see this it is certainly a bug. (or you are developing)")
